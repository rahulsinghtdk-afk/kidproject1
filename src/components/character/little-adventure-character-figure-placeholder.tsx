...
function mouthPathForMood(mood: PlaceholderFigureMood, clapping: boolean): string {
  if (mood === "celebrate" || clapping) {
    return "M 44 88 Q 60 106 76 88";
  }
  if (mood === "encouraging") {
    return "M 50 90 Q 60 96 70 90";
  }
  if (mood === "gentle-full") {
    return "M 52 92 Q 60 88 68 92";
  }
  if (mood === "requesting") {
    return "M 46 90 Q 60 102 74 90";
  }
  return "M 48 88 Q 60 98 72 88";
}

function HomeCharacterFigure() {
  return (
    <svg
      viewBox="0 0 120 140"
      className={FIGURE_SIZE_CLASS.home}
      role="img"
      aria-label=""
    >
      <ellipse
...
function PlayCharacterFigure({ mood, clapping, reducedMotion }: PlaceholderFigureProps) {
  const isClapping = clapping && !reducedMotion;
  const mouthPath = mouthPathForMood(mood, clapping);

  return (
    <svg
      viewBox="0 0 120 140"
      className={FIGURE_SIZE_CLASS.play}
      role="img"
      aria-label=""
    >
      <ellipse
...
export type { PlaceholderFigureMood, PlaceholderFigureProps };
